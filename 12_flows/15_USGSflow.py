# 15_USGSflow.py
# ------------------------------------------------------------
# Fetch USGS flow + stage data for all USGS sites in flows.csv
#
# Input:
#   • 100_Data/flows.csv
#
...
print("🌊 Step 15: Fetching USGS flow + stage data for all USGS sites in flows.csv...")
...
output_path = data_dir / "USGS_flows.csv"

if not flows_path.exists():
    raise FileNotFoundError(f"❌ flows.csv not found at: {flows_path}")

print(f"📂 Reading site definitions from: {flows_path}")
...
if missing:
    raise ValueError(f"❌ flows.csv is missing required columns: {missing}")
...
def normalize_gage_id(raw) -> str | None:
    """
    Normalize a gage ID from flows.csv to a USGS site_id string.
...
def iter_usgs_sites(flows: pd.DataFrame):
    """
    Yield (site_id, river, site_name) for every valid USGS site in flows.csv.
...
if not site_records:
    print("📭 No valid USGS sites found in flows.csv — nothing to fetch.")
    raise SystemExit(0)

print(f"✅ Found {len(site_records)} USGS site entries in flows.csv")
...
# ------------------------------------------------------------
if not all_frames:
    print("📭 No data retrieved for any site/window — USGS_flows.csv not created.")
    raise SystemExit(0)
...
print("\n🎯 Step 15 complete: USGS_flows.csv created.")