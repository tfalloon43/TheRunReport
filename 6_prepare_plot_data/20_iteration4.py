# 20_iteration4.py
# ------------------------------------------------------------
# Step 20: Full re-iteration of biological year metrics (v4)
#
# After additional row removals and cleanup (x3 condense),
...
#   - STEP 4 (by_short4) now only runs when Family ∈
#     ["Steelhead", "Chinook", "Coho", "Chum", "Pink", "Sockeye"]
#
# Input : 100_Data/csv_plotdata.csv
# Output: 100_Data/20_iteration4_output.csv + updates csv_plotdata.csv
# ------------------------------------------------------------

import pandas as pd
from pathlib import Path

print("🏗️ Step 20: Rebuilding biological metrics (day_diff4 → x_count4)...")
...
output_path = data_dir / "20_iteration4_output.csv"
...
# ============================================================
# STEP 1: day_diff4 and adult_diff4
# ============================================================
print("🔹 Calculating day_diff4 and adult_diff4...")
df["day_diff4"] = (
...
for i, row in df.iterrows():
    ...
    if keys != prev_keys:
        ...
        current_year += 1
    by_adult4.append(current_year)

df["by_adult4"] = by_adult4

# ============================================================
# STEP 3: by_adult4_length
# ============================================================
...
# ============================================================
# STEP 4: by_short4 (spillover short run detection)
# ============================================================
print("🔹 Detecting short spillover runs (by_short4, salmonid families only)...")
df["by_short4"] = ""
...
def flag_spillover_short_runs(g):
    ...
    for i, row in runs.iterrows():
        curr_len = row["by_adult4_length"]
        if curr_len > 15:
            lookahead = runs.iloc[i+1:i+5]
            for _, next_row in lookahead.iterrows():
                next_len = next_row["by_adult4_length"]
                next_by = next_row["by_adult4"]
...
# ============================================================
# STEP 5: x_count4 (contiguous X sequence count)
# ============================================================
...
# Summary
# ------------------------------------------------------------
print(f"✅ Iteration 4 complete → {output_path}")
print(f"📊 Recomputed columns: day_diff4, adult_diff4, by_adult4, by_adult4_length, by_short4, x_count4")
print(f"🔢 Total rows: {len(df):,}")
print(f"🔎 Short runs flagged (salmonids only): {(df['by_short4'] == 'X').sum():,}")
