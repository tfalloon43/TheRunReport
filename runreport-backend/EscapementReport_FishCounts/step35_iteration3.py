# step35_iteration3.py
# ------------------------------------------------------------
# Step 35: Biological Metrics Iteration 3
#
...
print("🏗️ Step 35: Recomputing biological metrics (Iteration 3)...")
...
df["adult_diff3"] = df.groupby(group_cols)["Adult Total"].diff()
...
df.loc[df["group_changed"], "adult_diff3"] = df.loc[df["group_changed"], "Adult Total"]
df["adult_diff3"] = df["adult_diff3"].fillna(df["Adult Total"])
    ...
    for i, r in runs.iterrows():
        ...
        if curr_len > 15:
            ...
            for _, nxt in lookahead.iterrows():
                next_len = nxt["by_adult3_length"]
                next_by = nxt["by_adult3"]
