# step37_iteration4.py
# ------------------------------------------------------------
# Step 37: Biological Metrics Iteration 4
#
...
print("🏗️ Step 37: Recomputing biological metrics (Iteration 4)...")
...
df["adult_diff4"] = df.groupby(group_cols)["Adult Total"].diff()
...
df.loc[df["group_changed"], "adult_diff4"] = df.loc[df["group_changed"], "Adult Total"]
df["adult_diff4"] = df["adult_diff4"].fillna(df["Adult Total"])
    ...
    for i, r in runs.iterrows():
        ...
        if curr_len > 15:
            ...
            for _, nxt in lookahead.iterrows():
                next_len = nxt["by_adult4_length"]
                next_by = nxt["by_adult4"]
